...
//
// IMPORTANT: This benchmark must be compiled with Clang to properly measure
// the LLVM backend optimization. The vector multiply operations use GCC-style
// vector extensions which Clang will lower through the LLVM backend where our
// optimization is applied.
//
// Build with: clang++ -O2 -march=native (or specific -march=x86-64-v3 for AVX2)
//
//===----------------------------------------------------------------------===//
...
// This benchmark must be compiled with Clang to properly measure LLVM codegen.
// The vector multiply operations use GCC-style vector extensions which Clang
// will lower through the LLVM backend where our optimization is applied.
//
// Manual build command (from llvm-project root, on Windows with MSVC build):
...
// Vector types using GCC vector extensions - Clang will lower these through
// the LLVM backend where our vXi8 multiply optimization is applied
...
// Using volatile to ensure the compiler doesn't optimize away the initialization
